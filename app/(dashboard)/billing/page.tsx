import { getSubscriptionsData, getSitesData, getManualAccessData, getActivityData } from '@/lib/dashboard-data'
...
export default async function BillingPage({ searchParams }: { searchParams: { site?: string; plan?: string } }) {
  const [subscriptions, sites, manualAccess, activity] = await Promise.all([getSubscriptionsData(), getSitesData(), getManualAccessData(), getActivityData()])
  ...
  return <div className="page-enter">
    ...
    <section id="manual-access"><div className="section-heading"><div><h2>Manual billing</h2><p>For apps with their own Supabase/Auth. Confirm payment, review the user's latest activity, and pause or restore each account by email.</p></div></div><ManualAccessForm sites={sites as any[]} /><ManualAccessTable entries={manualAccess as any[]} activity={activity as any[]} /></section>
  </div>
}
