...
export async function POST(request: Request) {
  ...
  const input = await request.json() as { siteId?: string; email?: string; plan?: string; paymentStatus?: string; accessOverride?: string; accessDurationDays?: number; priceCents?: number; price?: number; priceCurrency?: string }
  ...
  const currency = String(input.priceCurrency ?? 'usd').toLowerCase()
  const effectiveCurrency = currency === 'tzs' ? 'tzs' : 'usd'
  const priceValue = input.price !== undefined ? Number(input.price) : undefined
  ...
  const priceCents = input.priceCents !== undefined ? Number(input.priceCents) : priceValue !== undefined ? Math.round(priceValue * (effectiveCurrency === 'usd' ? 100 : 1)) : undefined
  if (priceCents !== undefined && (!Number.isFinite(priceCents) || priceCents < 0)) return NextResponse.json({ error: 'Enter a valid non-negative price in cents.' }, { status: 400 })
  ...
  if (priceCents !== undefined) {
    accessRecord.price_cents = priceCents
    accessRecord.price_currency = effectiveCurrency
  }
