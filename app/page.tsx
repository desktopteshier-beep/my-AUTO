...
export default async function Dashboard({ searchParams }: { searchParams: { site?: string; plan?: string } }) {
  ...
  return <div className="app-shell">
    ...
    <main><header className="topbar"><h1>Overview</h1><DashboardTools projects={projects.map(p => ({ id: p.id, name: p.name, domain: p.sites?.[0]?.domain }))} users={subscriptions.map((s: any) => ({ id: s.id, name: s.users?.email ?? '', email: s.users?.email }))} /></header>
      ...
      <section id="manual-access"><div className="section-heading"><div><h2>Manual project access</h2><p>Confirm payment, review the user's latest activity, and pause or restore each account.</p></div></div><ManualAccessForm sites={allSites as any[]} /><ManualAccessTable entries={manualAccess as any[]} activity={activity as any[]} /></section>
