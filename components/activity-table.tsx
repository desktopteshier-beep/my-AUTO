...
export function activityLabel(event: string) { return event === 'page_view' ? 'Visited' : event === 'signed_in' ? 'Signed in' : 'Used feature' }
export function ActivityTable({ activity }: { activity: Activity[] }) {
  return <div className="table-wrap"><table><thead><tr><th>Time</th><th>Site</th><th>User / visitor</th><th>Activity</th><th>Page</th></tr></thead><tbody>
    {activity.map(item => { const who = item.user_email ?? (item.anonymous_id ? `Visitor ${item.anonymous_id.slice(0, 8)}` : 'Anonymous visitor'); return <tr key={item.id}><td>{new Date(item.created_at).toLocaleString()}</td><td>{item.sites?.name ?? 'Unknown site'}</td><td><span className="who-cell"><Avatar name={who} />{who}</span></td><td>{activityLabel(item.event_type)}</td><td>{item.path ?? '—'}</td></tr> })}
