...
export function DashboardTools({ projects, users }: { projects: Item[]; users: Item[] }) {
  ...
  return <><div className="actions"><button className="secondary command-button" onClick={() => setOpen(true)}><span aria-hidden="true">⌕</span>Search anything…<kbd>⌘K</kbd></button><button className="secondary" onClick={() => { router.refresh(); setToast('Dashboard refreshed') }}>Refresh</button><a className="primary action-link" href="/projects/new">Add project</a></div>{open && <div className="palette-backdrop" onMouseDown={() => setOpen(false)}><div className="palette" onMouseDown={event => event.stopPropagation()}><input autoFocus value={query} onChange={e => setQuery(e.target.value)} placeholder="Search projects, users, or pages…" /><div className="palette-label">Quick jump</div><a href="/">Overview</a><a href="/deploys">Deploys</a><a href="/monitoring">Monitoring</a><a href="/users">Users</a><a href="/billing">Billing</a><a href="/settings">Settings</a><div className="palette-label">Results</div>{results.map(item => <button key={`${item.kind}-${item.id}`} onClick={() => { setOpen(false); router.push(item.kind === 'Project' ? '/deploys' : '/users') }}><span>{item.name}</span><small>{item.kind}{item.domain ? ` · ${item.domain}` : ''}</small></button>)}{!results.length && <p className="palette-empty">No matching projects or users.</p>}</div></div>}{toast && <div className="toast"><span>✓</span>{toast}</div>}</>
}
