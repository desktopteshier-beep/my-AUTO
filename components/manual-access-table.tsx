...
export function ManualAccessTable({ entries, activity, usageDays }: { entries: Access[]; activity: Activity[]; usageDays?: Record<string, number> }) {
  ...
  return <div className="table-wrap"><table><thead><tr><th>User email</th><th>Project</th><th>Plan</th><th>Payment</th><th>Days active</th><th>Last use</th><th>Access</th></tr></thead><tbody>{entries.map(item => {
    ...
    return <tr key={item.id}><td>{item.email}</td><td>{item.sites?.name}</td><td>{item.plan}</td><td>{item.payment_status}</td><td className="numeric">{days ? `${days} day${days === 1 ? '' : 's'}` : '—'}</td><td>{lastUse ? `${lastUse.event_type.replace('_', ' ')} · ${new Date(lastUse.created_at).toLocaleString()}` : 'No tracked use yet'}</td><td><button className="access-button" disabled={busy === item.id} onClick={() => toggle(item)}>{busy === item.id ? 'Saving…' : item.access_override === 'paused' ? 'Restore access' : 'Pause access'}</button></td></tr>
  })}{!entries.length && <tr><td colSpan={7} className="empty"><strong>No manual access records</strong><p>Add a user above to control access for an existing project.</p></td></tr>}</tbody></table></div>
}
