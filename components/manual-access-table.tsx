...
export function ManualAccessTable({ entries, activity, usageDays }: { entries: Access[]; activity: Activity[]; usageDays?: Record<string, number> }) {
  ...
  return <div className="table-wrap"><table><thead><tr><th>User email</th><th>Project</th><th>Plan</th><th>Payment</th><th>Price</th><th>Expires</th><th>Days active</th><th>Last use</th><th>Access</th></tr></thead><tbody>{entries.map(item => {
    ...
    return <tr key={item.id}><td>{item.email}</td><td>{item.sites?.name}</td><td>{item.plan}</td><td>{item.payment_status}</td><td>{item.price_cents != null ? formatMoney(item.price_cents, item.price_currency) : '—'}</td><td><form onSubmit={event => { event.preventDefault(); const value = Number(new FormData(event.currentTarget).get('days')); if (value > 0) extend(item, value) }} style={{ display: 'flex', gap: 4, alignItems: 'center' }}><span>{expiryLabel(item)}</span><input name="days" type="number" min="1" step="1" placeholder="days" style={{ width: 64 }} /><button className="secondary" type="submit" disabled={busy === item.id}>Set</button></form></td><td className="numeric">{days ? `${days} day${days === 1 ? '' : 's'}` : '—'}</td><td>{lastUse ? `${lastUse.event_type.replace('_', ' ')} · ${new Date(lastUse.created_at).toLocaleString()}` : 'No tracked use yet'}</td><td><button className="access-button" disabled={busy === item.id} onClick={() => toggle(item)}>{busy === item.id ? 'Saving…' : item.access_override === 'paused' ? 'Restore access' : 'Pause access'}</button></td></tr>
