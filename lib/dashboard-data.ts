...
export async function getDashboardData() {
  ...
  const [{ data: projects, error: projectError }, { data: subscriptions, error: subscriptionError }] = await Promise.all([
    db.from('projects').select('id,name,github_owner,github_repo,deploy_target,monitoring_provider,monitoring_check_id,sentry_project_slug,site_id,sites(name,domain)').order('name'),
  ...
  const health = await Promise.all(((projects ?? []) as Project[]).map(async project => ({ ...project, deploy: await deploymentState(project), uptime: await uptimeState(project), errors: await errorCount(project) })))
